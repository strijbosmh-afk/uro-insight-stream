...
export function GroupsTab({ filters }: { filters: DiscoverFilterState }) {
  const { user } = useAuth();
  ...
  const { data: groups = [], isLoading } = useQuery({
    queryKey: ["groups", { areaId, search: filters.query, sort }],
    queryFn: () =>
      fetchGroups({
        data: {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        {areas.map((a) => (
          <ChipButton key={a.id} active={areaId === a.id} onClick={() => setAreaId(a.id)}>
            {toTitleCase(a.name)}
          </ChipButton>
        ))}
      ...
      {isLoading ? (
        ...
        <Panel title="No groups found">
          <p className="text-[12px] text-text-muted">
            No groups match. Try a different cancer area or clear the search.
          </p>
        </Panel>
      ) : (
...
function GroupCard({
  group, pending, onSubscribe, onUnsubscribe,
}: {
  group: GroupSummary;
  pending: boolean;
  onSubscribe: () => void;
  ...
  return (
    <div className={
      ...
      (subscribed ? "bg-accent/10 border-accent/40 text-text-primary" : "bg-panel border-border hover:border-accent/40")
    }>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {subscribed && (
            <Badge variant="outline" className="gap-1 text-[10px] uppercase tracking-wider border-accent/60 bg-accent/15 text-accent">
              <BadgeCheck className="w-3 h-3" /> Subscribed
            </Badge>
          )}
          {group.visibility === "official" && (
            <Badge variant="outline" className="gap-1 text-[10px] uppercase tracking-wider border-accent/40 text-accent">
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {group.cancer_areas.slice(0, 3).map((a) => (
          <span key={a.id} className="text-[10px] font-mono uppercase tracking-wider text-text-muted bg-panel-elevated/50 border border-border px-1.5 py-0.5 rounded">
            {a.name}
          </span>
        ))}
      </div>
